...
impl Queue {
    ...
    /// jobs, etc).
    pub fn tick<T>(&self, now: T) -> Result<()>
        where T: Into<Timestamp> + Copy + std::fmt::Debug,
    {
        let now = now.into();
    ...
    pub fn enqueue<C, P, T>(&self, channel_name: C, job_data: Vec<u8>, priority: P, ttr: u32, delay: Option<T>, now: T) -> Result<JobID>
        where C: Into<String> + std::fmt::Debug,
              P: Into<Priority> + Copy + std::fmt::Debug,
              T: Into<Timestamp> + Copy + std::fmt::Debug,
    {
    ...
    #[tracing::instrument(skip(self))]
    pub fn release<P, T>(&self, job_id: &JobID, priority: P, delay: Option<T>) -> Result<JobID>
        where P: Into<Priority> + std::fmt::Debug,
              T: Into<Timestamp> + std::fmt::Debug,
    {
...
mod tests {
    ...
    fn enqueue() {
        ...
        queue1.enqueue("blixtopher", Vec::from("get a job".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        ...
        queue1.enqueue("blixtopher", Vec::from("say hello".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        assert_eq!(queue1.channels.len(), 1);
        assert_eq!(queue1.channels.contains_key("blixtopher"), true);
        assert_eq!(queue1.channels.get("blixtopher").unwrap().metrics().ready(), 2);
        assert_eq!(queue1.channels.get("blixtopher").unwrap().metrics().delayed(), 0);
        queue1.enqueue("WORK", Vec::from("get a job".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        ...
        queue2.enqueue("blixtopher", Vec::from("get a job".as_bytes()), 1024, 300, Some(696969696), 0.into()).unwrap();
    ...
    fn dequeue() {
        let queue1  = make_queue(32);
        queue1.enqueue("videos", Vec::from("process-vid1".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("videos", Vec::from("process-vid2".as_bytes()), 1000, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("videos", Vec::from("process-vid3".as_bytes()), 1000, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("downloads", Vec::from("https://post.vidz.ru/videos/YOUR-WIFE-WITH-THE-NEIGHBOR.rar.mp4.avi.exe".as_bytes()), 1001, 300, None::<Timestamp>, 0.into()).unwrap();
    ...
    fn restore() {
        let queue1 = make_queue(32);
        let job1 = queue1.enqueue("jobs", Vec::from("do stuff".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue1.enqueue("jobs", Vec::from("do something".as_bytes()), 0, 300, None::<Timestamp>, 0.into()).unwrap();
        let job3 = queue1.enqueue("jobs", Vec::from("do something else".as_bytes()), 0, 300, None::<Timestamp>, 0.into()).unwrap();
        let job4 = queue1.enqueue("jobs", Vec::from("do later".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job5 = queue1.enqueue("todo", Vec::from("make todo list".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        // "i have done that," said toad
        let job6 = queue1.enqueue("todo", Vec::from("wake up".as_bytes()), 1024, 300, Some(4000), 0.into()).unwrap();
        for i in 0..9000 {
            queue1.enqueue("chain", Vec::from(format!("send this queue to {} people or else", i).as_bytes()), 2000, 300, None::<Timestamp>, 0.into()).unwrap();
        }
    ...
    fn dequeue_job() {
        ...
        let job1 = queue.enqueue("jobs", Vec::from("jerry".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue.enqueue("jobs", Vec::from("larry".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job3 = queue.enqueue("jobs", Vec::from("barry".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job4 = queue.enqueue("jobs", Vec::from("mary".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job5 = queue.enqueue("jobs", Vec::from("harry".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job6 = queue.enqueue("jobs", Vec::from("dary".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
    ...
    fn dequeue_parallel_strict() {
        let num_threads = 128;
        let queue1 = make_queue(32);
        let job1_id = queue1.enqueue("vids1", Vec::from("threw rocks at my car".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2_id = queue1.enqueue("vids2", Vec::from("we don't have any kids".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job3_id = queue1.enqueue("vids3", Vec::from("went to the house of the kids".as_bytes()), 1000, 300, None::<Timestamp>, 0.into()).unwrap();
        ...
        for pri in [1024, 1000, 100, 2000].iter() {
            for i in 0..5 {
                for chan in ["vids1", "vids2", "vids3"].iter() {
                    queue2.enqueue(*chan, Vec::from(format!("{}/{}", pri, i).as_bytes()), *pri, 300, None::<Timestamp>, 0.into()).unwrap();
                }
            }
        }
    ...
    fn release() {
        ...
        queue1.enqueue("videos", Vec::from("process-vid2".as_bytes()), 1000, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("videos", Vec::from("process-vid3".as_bytes()), 1000, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("downloads", Vec::from("https://big-files.com/document".as_bytes()), 1001, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("downloads", Vec::from("https://big-files.com/BIG-document".as_bytes()), 1001, 300, None::<Timestamp>, 0.into()).unwrap();
    ...
    fn delete() {
        let queue1 = make_queue(32);
        let job1 = queue1.enqueue("vidz", Vec::from("record".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue1.enqueue("vidz", Vec::from("edit".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job3 = queue1.enqueue("vidz", Vec::from("show".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job4 = queue1.enqueue("vidz", Vec::from("critique".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job5 = queue1.enqueue("vidz", Vec::from("sob-uncontrollably".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
    ...
    fn fail_reserved() {
        ...
        let job1 = queue1.enqueue("vidz", Vec::from("record".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue1.enqueue("vidz", Vec::from("edit".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job3 = queue1.enqueue("vidz", Vec::from("watch".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job4 = queue1.enqueue("vidz", Vec::from("critique".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job5 = queue1.enqueue("vidz", Vec::from("sob-uncontrollably".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
    ...
    fn kick_failed_delayed() {
        let queue1 = make_queue(32);
        let _job1 = queue1.enqueue("prank-calls", Vec::from("i'm detective john kimble".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let _job2 = queue1.enqueue("prank-calls", Vec::from("yeah sure you are".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job3 = queue1.enqueue("prank-calls", Vec::from("i'm a cop you idiot".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job4 = queue1.enqueue("prank-calls", Vec::from("well how 'bout i come over there and i whoop your god damn ass??".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
    ...
    fn kick_job() {
        let queue1 = make_queue(32);
        let job1 = queue1.enqueue("prank-calls", Vec::from("i'm detective john kimble".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue1.enqueue("prank-calls", Vec::from("yeah sure you are".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job3 = queue1.enqueue("prank-calls", Vec::from("i'm a cop you idiot".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job4 = queue1.enqueue("prank-calls", Vec::from("well how 'bout i come over there and i whoop your god damn ass??".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job1_2 = queue1.dequeue(&vec!["prank-calls".into()], Ordering::Strict).unwrap().unwrap();
        let job2_2 = queue1.dequeue(&vec!["prank-calls".into()], Ordering::Strict).unwrap().unwrap();
    ...
    fn tick() {
        ...
        let job1 = queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, Some(6000), 0.into()).unwrap();
        queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, Some(6000), 0.into()).unwrap();
        queue1.enqueue("jobs", Vec::from("hello".as_bytes()), 1024, 300, Some(6000), 0.into()).unwrap();
    ...
    fn peek_ready() {
        ...
        queue1.enqueue("prank-calls1", Vec::from("i'm detective john kimble".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        queue1.enqueue("prank-calls1", Vec::from("yeah sure you are".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("prank-calls2", Vec::from("i'm a cop you idiot".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        queue1.enqueue("prank-calls2", Vec::from("well how 'bout i come over there and i whoop your god damn ass??".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
    ...
    fn peek_failed() {
        let queue1 = make_queue(4);
        let _job1 = queue1.enqueue("prank-calls1", Vec::from("i'm detective john kimble".as_bytes()), 1024, 300, Some(5000), 0.into()).unwrap();
        let job2 = queue1.enqueue("prank-calls1", Vec::from("yeah sure you are".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let _job3 = queue1.enqueue("prank-calls2", Vec::from("i'm a cop you idiot".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let _job4 = queue1.enqueue("prank-calls2", Vec::from("well how 'bout i come over there and i whoop your god damn ass??".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let _sub = queue1.subscribe("get-a-job").unwrap();
    ...
    fn list_channels() {
        ...
        let _job1 = queue1.enqueue("prank-calls1", Vec::from("i'm detective john kimble".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job2 = queue1.enqueue("prank-calls2", Vec::from("yeah sure you are".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let _job3 = queue1.enqueue("prank-calls3", Vec::from("i'm a cop you idiot".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
        let job4 = queue1.enqueue("prank-calls4", Vec::from("well how 'bout i come over there and i whoop your god damn ass??".as_bytes()), 1024, 300, None::<Timestamp>, 0.into()).unwrap();
