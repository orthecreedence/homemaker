...
mod tests {
    ...
    macro_rules! test_cmd_bare {
        ($variant:ident, $name:expr, $misspell:expr) => {{
            {
                let (rest, cmd) = parse_request(concatcp!($name, "\r\n").as_bytes()).unwrap();
                assert_eq!(rest, &[]);
    ...
    macro_rules! test_cmd_u64 {
        ($variant:ident, $name:expr, $misspell:expr) => {{
            {
                let (rest, cmd) = parse_request(concatcp!($name, " 18446744073709551615\r\n").as_bytes()).unwrap();
                assert_eq!(rest, &[]);
                assert_eq!(cmd, Request::$variant(18_446_744_073_709_551_615));
            }
            {
                let (rest, cmd) = parse_request(concatcp!($name, " 0\r\nget a job\r\n").as_bytes()).unwrap();
                assert_eq!(rest, b"get a job\r\n");
                assert_eq!(cmd, Request::$variant(0));
            }

            assert_eq!(parse_request(concatcp!($misspell, " 123\r\n").as_bytes()).unwrap_err(), Error::UnknownRequest($misspell));
    ...
    macro_rules! test_cmd_chan {
        ($variant:ident, $name:expr, $misspell:expr) => {{
            {
                let (rest, cmd) = parse_request(concatcp!($name, " intake.api.delicious_beans\r\n").as_bytes()).unwrap();
                assert_eq!(rest, &[]);
                assert_eq!(cmd, Request::$variant("intake.api.delicious_beans"));
            }
            {
                let (rest, cmd) = parse_request(concatcp!($name, " $fun-facts+about//sal;mon.and_a(brand-new).dance\r\nget a job\r\n").as_bytes()).unwrap();
                assert_eq!(rest, b"get a job\r\n");
                assert_eq!(cmd, Request::$variant("$fun-facts+about//sal;mon.and_a(brand-new).dance"));
            }
            {
                let req = format!("{} {}\r\n", $name, bad_channel_name());
                assert_eq!(parse_request(req.as_bytes()).unwrap_err(), Error::BadChannel);
            }

            assert_eq!(parse_request(concatcp!($misspell, " get-a-job\r\n").as_bytes()).unwrap_err(), Error::UnknownRequest($misspell));
    ...
    fn pause_tube() {
        {
            let (rest, cmd) = parse_request(b"pause-tube $snacky-smores.123 4294967295\r\n").unwrap();
            assert_eq!(rest, &[]);
            assert_eq!(cmd, Request::PauseTube("$snacky-smores.123", 4_294_967_295));
        }
        {
            let (rest, cmd) = parse_request(b"pause-tube my-channel 5\r\ntesting\r\n").unwrap();
            assert_eq!(rest, b"testing\r\n");
        ...
        assert_eq!(parse_request(b"pause-teub frisky 1231\r\n").unwrap_err(), Error::UnknownRequest("pause-teub"));
    ...
    fn put() {
        {
            let (rest, cmd) = parse_request(b"put 4294967295 4294967295 4294967295 9\r\nget a job\r\n").unwrap();
            assert_eq!(rest, &[]);
    ...
    fn release() {
        {
            let (rest, cmd) = parse_request(b"release 18446744073709551615 4294967295 4294967295\r\n").unwrap();
            assert_eq!(rest, &[]);
            assert_eq!(cmd, Request::Release(18_446_744_073_709_551_615, 4_294_967_295, 4_294_967_295));
        }
        {
            let (rest, cmd) = parse_request(b"release 123 456 0011222\r\ntesting\r\n").unwrap();
            assert_eq!(rest, b"testing\r\n");
    ...
    fn parse_multi() {
        ...
        let (rest, cmd1) = parse_request(multi).unwrap();
        assert_eq!(cmd1, Request::Use("barky"));
        let (rest, cmd2) = parse_request(rest).unwrap();
        assert_eq!(cmd2, Request::Put(1024, 0, 300, b"bark bark bark!"));
        let (rest, cmd3) = parse_request(rest).unwrap();
        assert_eq!(cmd3, Request::Watch("barky"));
        let (rest, cmd4) = parse_request(rest).unwrap();
        assert_eq!(cmd4, Request::Watch("dogs"));
        let (rest, cmd5) = parse_request(rest).unwrap();
        assert_eq!(cmd5, Request::Reserve);
        let (rest, cmd6) = parse_request(rest).unwrap();
